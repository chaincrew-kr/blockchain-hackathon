...
- (다음 PR) `feat/escrow-fund-flow → dev` — authority 서명 검증(init_escrow) + settle_batch 회차 단위 재설계(이슈 #6)
...
5. **A(프론트) 공유 필요**: `init_escrow`가 이제 `authority` 서명을 요구하므로, 에스크로 생성 플로우에서 정산 에이전트 키페어가 그 시점에 서명 가능해야 함 — IDL은 이미 최신 반영해서 `packages/schema/idl`에 올려둠
6. `docs/ponyo_work/DEADLINE.md`(D, 7/31 D-3 항목)의 **Devnet 배포**는 아직 localnet만 해놓은 상태 — `Anchor.toml` 주석("Devnet 이전은 8/1 이후")과 상충하니 팀 확인 필요
