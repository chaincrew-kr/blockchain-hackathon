...
| 2026-08-01 | `ruleHash` 인코딩 확정 — `sha256("{ruleVersion}\|{theaterBps}\|{distributorBps}\|{distributionFeeBps}")`(TicketEvent 해시체인과 동일한 "\|" join 방식). `settle_batch.rs`에 검증 추가, `apps/web/src/lib/hash.ts`에 `computeRuleHash()` 헬퍼 추가. SettlementRule 전체(조항·충돌·승인 여부)는 검증 대상에서 제외 — 온체인 인자로 넘어가지 않아 재현 불가능 | 결정 완료 (A·B·D) |
