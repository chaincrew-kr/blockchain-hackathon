...
| `ruleHash`      | `rule_hash: [u8; 32]`     | A·B·D 합의     | 인코딩 확정 (2026-08-01, §12 참고) — `settle_batch.rs`에 검증 구현 |
...
| 할인·쿠폰 거래 | `amount`만 있어 할인 원인 추적 불가    | 거래 유형·정가·실결제액 필드 | A·B·D     |
| 유효매출       | 회차 순매출로 단순 계산                | 계약 규칙 기반 산정 위치     | A·B·D     |
| 데이터 입력원  | Demo fixture                           | Excel·발권 API 공통 어댑터   | A·D       |

2026-07-31 씨네큐브 현장 답변에서 개별 거래의 유료·무료·할인·환불 여부가
구분되고, 유효매출 기준은 극장별로 다를 수 있음이 확인됐다. 후보 필드와 조사 한계는
[현장 조사 문서](research/CINECUBE_FIELD_RESPONSE_2026-07-31.md)를 기준으로
검토한다. 원본 Excel·API 명세를 받기 전에는 필드를 확정하지 않는다.

**해결됨(§12 변경 기록 참고)**: 권리자 잔액(이슈 #6 — Allocation PDA 구조로 해결), `ruleHash` 인코딩(아래).
...
| 2026-07-31 | 권리자 잔액(이슈 #6) — `balances: []` 대신 역할별(최대 4개) Allocation PDA(`[b"allocation", movie_id, role_byte]`)를 D가 직접 조회하는 방식으로 확정, 별도 필드 불필요                                                                                                                                                                                                                                                                                          | 결정 완료 (B·D)       |
| 2026-08-01 | `ruleHash` 인코딩 확정 — `sha256("{ruleVersion}\|{theaterBps}\|{distributorBps}\|{distributionFeeBps}\|{investorProfitBps}")`(TicketEvent 해시체인과 동일한 "\|" join 방식). `settle_batch.rs`에 검증 추가, `apps/web/src/lib/hash.ts`에 `computeRuleHash()` 헬퍼 추가. SettlementRule 전체(조항·충돌·승인 여부)는 검증 대상에서 제외 — 온체인 인자로 넘어가지 않아 재현 불가능                                                                                 | 결정 완료 (A·B·D)     |
| 2026-08-01 | (제안) 할인·쿠폰 거래 — 실제 데모 계약서 제11조 "정산 기준액은 정가가 아닌 실결제액(ACTUAL_PAID)" + "차액은 부금율대로 안분 부담"을 근거로, `TicketEvent.amount`(실결제액) 하나만 유지하고 정가·할인율 필드는 추가하지 않는 안. `settle_batch`가 `amount`를 그대로 기존 `theater_bps`/`distributor_bps`로 나누므로 할인 차액은 이미 부금율대로 자동 안분됨 — 별도 회계 처리 불필요. 무료(0원)와 할인은 `amount === 0` 여부로 이미 구분됨. A 제안, B·D 확인 대기 | 팀 리뷰 대기 (A 제안) |
